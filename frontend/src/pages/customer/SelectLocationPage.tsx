...
import { MapPin, Search, Navigation, Clock, ChevronRight, Store, X, LocateFixed, AlertTriangle, Truck, ShoppingBag } from 'lucide-react';
...
export const SelectLocationPage: React.FC = () => {
  ...
  const checkEligibilityWithBackend = useCallback(async (lat?: number, lng?: number, pc?: string, branchList?: Branch[]) => {
    ...
    try {
      ...
      if (eligible) {
        ...
        setFulfillmentType((prev) => (prev === 'DELIVERY' ? 'DELIVERY' : prev));
      } else {
        // Outside 2-mile radius: Force collection default & recommend nearest store
  ...
  const handleSelectDelivery = () => {
    if (!isDeliveryEligible || (distanceMiles !== null && distanceMiles > 2.0)) {
      return; // Disabled and non-clickable when > 2 miles
    }
    ...
    <div className="w-full max-w-[1160px] mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-10 pb-20 text-[#F5F5F5]">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start mb-8">
        ...
        <div className="lg:col-span-5 space-y-6">
          ...
          {/* Location Error / Permission Denied Message */}
          {locationError && (
          ...
          <div className="bg-[#0D0D0D] border border-[#242424] rounded-[10px] p-5 space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              {/* 2. COLLECTION OPTION (Always available) */}
              <div
